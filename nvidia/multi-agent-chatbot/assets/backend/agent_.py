...
from python_sandbox import safe_exec_python  # python code execution
...
class ChatAgent:
    ...
    async def generate(self, state: State) -> Dict[str, Any]:
        ...
        # EDIT: parsing for code output
        try:
            parsed = json.loads(raw_output)
            response_text = parsed.get("response_text", "")
            python_code = parsed.get("python_code", "")
            should_execute = parsed.get("should_execute", False)
        ...
        # Only execute code if should_execute is True and code is not empty
        if should_execute and python_code.strip():
            execution_result = safe_exec_python(python_code)
